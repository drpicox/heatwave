...
def histogram(values: pd.Series, lo: int, hi: int) -> list[int] | None:
    """Histograma dispers d'una serie de temperatures.

    Torna `[offset, c0, c1, ...]`, on `offset` es la temperatura entera del
    primer bin no buit. Els valors fora de [lo, hi) s'acumulen als bins extrems
    perque no es perdi cap dia del recompte total.
    """
    v = values.dropna().to_numpy()
    if v.size == 0:
        return None
    binned = np.clip(np.floor(v).astype(int), lo, hi - 1)
    counts = np.bincount(binned - lo, minlength=hi - lo)
    nz = np.nonzero(counts)[0]
    first, last = int(nz[0]), int(nz[-1])
    return [lo + first, *(int(c) for c in counts[first : last + 1])]


def count_at_or_above(hist: list[int] | None, threshold: int) -> int:
    """Mateixa aritmetica que fara el navegador. Serveix per als tests."""
    if not hist:
        return 0
    offset, counts = hist[0], hist[1:]
    return sum(c for i, c in enumerate(counts) if offset + i >= threshold)


def count_below(hist: list[int] | None, threshold: int) -> int:
    if not hist:
        return 0
    offset, counts = hist[0], hist[1:]
    return sum(c for i, c in enumerate(counts) if offset + i < threshold)
